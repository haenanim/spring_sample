    ...
    <ul class="list-group">
        <c:forEach var="news" items="${newsList}" varStatus="status">
            <li class="list-group-item list-group-action d-flex justify-content-between align-item-center">
                [${status.count}]
                <a href="/news?action=getNews&aid=${news.aid}" class="text-decoration-none">${news.title}</a>
                    ${news.date}
                <a href="/news?action=delNews&aid=${news.aid}"><span class="badge bg-secondary">&times;</span></a>
            </li>
        </c:forEach>
    </ul>
</div>
